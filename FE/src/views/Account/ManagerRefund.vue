<template>
  <fragment>
    <div class="col-lg-9 col-md-12">
      <h1 class="dash__h1 u-s-m-b-30">Đơn Hàng Trả Hàng</h1>
          ...
          <div class="manage-o">
            <div class="manage-o__header u-s-m-b-30">
              ...
              <div class="manage-o__text u-c-secondary">
                ...
                Trả hàng vào {{ product.ngaydelivery }}
              </div>
            </div>
            <!-- <div class="dash-l-r">
              <div class="manage-o__text u-c-secondary">
      ...
      >
        <h3>
          <i class="far fa-calendar-alt"></i> Lịch Trình/Cập Nhật Đơn Hoàn Trả
        </h3>
        <div
          class="express"
          v-if="trahang.length > 0"
          :style="{ height: heightProgress }"
        >
          <el-steps direction="vertical" :active="indexV">
            <el-step
              v-for="(item, index) in trahang"
              :key="index"
              :title="item.name"
            ></el-step>
          </el-steps>
...
export default {
            ...
            $(elStep[val.length - 1]).ready(() => {
              for (let i = 0; i < val.length; i++) {
                ...
                p.ready(() => {
                  ...
                  if (pexists.length == 0) {
                    $(p).append(
                      `<p class="content-pro">${val[i].content}</p>`
                    );
                    $(p).append(
                      `<p class="ngaygio"><i class="fas fa-clock"></i>  ${val[i].ngaygio}</p>`
                    );
                  }
                  this.heightProgress = sumi + 80 + "px";
                  this.heightWrapper = sumi + 180 + "px";
                  if (val[i].name === "THỦ TỤC HOÀN TIỀN HOÀN TẤT") {
  ...
  methods: {
    setSubcribe() {
      this.sockets.subscribe("khautrahang", (data) => {
        if (data) {
          this.notification();
          this.getTraHang();
        }
    ...
    notifiGiaoTC() {
      this.$message({
        ...
        message: `Mã đơn hàng ${this.product.madonhang} của bạn đã hoàn trả thành công`,
        type: "success",
        duration: 5000,
      });
      this.indexV = 0;
      for (let i = 0; i < this.trahang.length; i++) {
        setTimeout(() => {
          this.indexV++;
        }, 800);
      }
    ...
    setDefault() {
      ...
      var item = JSON.parse(localStorage.getItem("trahang"));
    ...
    getTraHang() {
      ...
      getApi("/getprogressrefund", body, (result) => {
    ...
    setUp(data) {
      data.forEach((e) => {
        if (e.trangthai.includes(";")) {
          var sp = e.trangthai.split(";");
          this.trahang.push({
            name: sp[0].toLocaleUpperCase(),
            content: sp[1],
            ngaygio: e.ngaygio,
          });
        } else {
          this.trahang.push({
            name: e.trangthai,
            content: "Yêu cầu trả hàng đã được gửi thành công",
            ngaygio: e.ngaygio,
          });
        }
      });
