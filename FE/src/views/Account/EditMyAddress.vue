      ...
      >
        <div class="dash__pad-2">
          ...
          <span class="dash__text u-s-m-b-30"
            >Chỉnh sửa lại thông tin địa chỉ giao hàng chính xác, giúp việc giao
            hàng được thuận lợi</span
          >
          <form class="dash-address-manipulation">
            <div class="gl-inline">
              <div class="u-s-m-b-30">
                <label class="gl-label" for="address-phone">Họ và Tên</label>
            ...
            <button
              class="btn btn--e-brand-b-2 goback"
...
export default {
  ...
  mounted() {
    ...
    this.address.diachithuongdung === 'VĂN PHÒNG' ? (this.address.setcolorvp = true) : (this.address.setcolorvp = false)
    this.address.diachithuongdung === 'NHÀ RIÊNG' ? (this.address.setcolornr = true) : (this.address.setcolornr = false)
    if (user) {
      this.user = user;
    }
  },
  methods: {
    ...
    onAddAddress() {
      ...
      getApi("/editaddress", body, (result) => {
